...
const contaAsHoras = setInterval(function(){             //essa constante possui uma função que puxa para dentro do java a hr ATUAL!!!
    const agora = new Date();                            //faz parte da function
    const timeStampAtual = agora.getTime();              //faz parte da function

    const dias = 1000 * 60 * 60 * 24;                    //transforma os milissegundo em dias
    const horas = 1000 * 60 * 60;                        //transforma os milissegundo em horas
    const minutos = 1000 * 60;                           //transforma os milissegundos em minutos
    ...
    if(distanciaAteEvento < 0){                                                       //essa linha (conjuto), cria uma condição que se a data do evento ja tiver passado, entao ele sobrera uma mudança na estrutura
